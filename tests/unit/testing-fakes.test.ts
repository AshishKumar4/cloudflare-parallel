...
import { poolFake, actorFake, schedulerFake } from '../../src/api/testing';
...
describe('poolFake', () => {
  ...
  it('passes typed bindings through env', async () => {
    const pool = poolFake<{ MULT: number }>({ bindings: { MULT: 3 } });
...
describe('actorFake', () => {
  it('returns a canonical IActorHandle', () => {
    const actor: IActorHandle<{ count: number }, Record<string, unknown>> = actorFake<
      { count: number },
...
describe('schedulerFake', () => {
  it('returns a canonical IScheduler', () => {
    const sched: IScheduler = schedulerFake({ id: 's0' });
    void sched;
  });

  it('runs an enqueued job to completion', async () => {
    const sched = schedulerFake({ id: 's' });
