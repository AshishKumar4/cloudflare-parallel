...
// Re-export the library DO classes so wrangler can register them.
...
const researchFn = async (spec: SourceSpec, env: { AI: Ai }): Promise<SourceResult> => {
  ...
  if (spec.type === 'wikipedia') {
    ...
    content = (data.hits || [])
      .slice(0, 5)
      .map((h: Record<string, unknown>) => `${h.title} (${h.points ?? 0} pts) ${h.url ?? ''}`)
      .join('\n');
...
async function streamResults(
  ...
  (async () => {
    ...
    for await (const { index, value } of pool.mapStream(researchFn as any, SOURCES, {
      context: { query },
    })) {
      sources.push(value as SourceResult);
      await writer.write(
        encoder.encode(`data: ${JSON.stringify({ type: 'source', index, result: value })}\n\n`),
      );
    }
